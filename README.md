...
The core mechanism: **artists, or the label they choose to work with, host their own audio files, images, and other artifacts, and own their own serving and analytics.** endonend never sits in that revenue or storage path; instead it functions like a union, aggregating independent artists to give them collective reach and discoverability without ever intermediating their work or income. To make that as convenient as a closed platform, endonend is both an open **protocol** (so artist-hosted catalogs can be published, discovered, and played in a standard way) and a **centralized reference experience** (a search, discovery, and playback surface built on that protocol) that any independently run instance can also implement, since the whole system is open source and self-hostable.
...
| `cli/` | Go CLI that generates, signs, and validates an artist/label manifest ([`KB/0004`](./KB/0004-endtoend-artist-cli.md)) |
| `api/` | Union Platform catalog API (planned) |
| `web/` | Web frontend: discovery site and generated artist storefronts (planned) |
| `mobile/` | Native iOS and Android clients (planned) |
| `data/` | Union Platform persistence layer (planned) |
...
- **Union Platform**: the central reference instance, a crawler/indexer that fetches and verifies artist manifests over plain HTTP, a catalog API, search/discovery, and the web and mobile clients. It never stores artist audio or image files, only metadata and pointers.
- **Protocol**: the manifest format and fetch-and-verify rules ([`KB/0003-manifest.md`](./KB/0003-manifest.md)) that let any artist's static files interoperate with any Union Platform instance.

**Languages and stack:**

- **Union Platform backend** (crawler, catalog API, search sync): Go, for small images and concurrent manifest fetching/verification.
...
```sh
cd cli
go build -o endtoend-artist-cli ./cmd
./endtoend-artist-cli              # launches an interactive menu
```

Or run individual commands directly:

```sh
./endtoend-artist-cli generate                # sign a manifest.json + history.json from union.source.json
./endtoend-artist-cli validate manifest.json  # validate a manifest
./endtoend-artist-cli keygen --url https://example.com/artist
./endtoend-artist-cli help                    # full command and flag reference
```

See [`KB/0004-endtoend-artist-cli.md`](./KB/0004-endtoend-artist-cli.md) for the CLI's design and [`KB/0003-manifest.md`](./KB/0003-manifest.md) for the manifest format it produces.
