...
- The manifest-generator/signer CLI tool's design is now defined in [0004-endonend-artist-cli.md](./0004-endonend-artist-cli.md); the remaining open item there is whether it should proactively notify previously-attested parties after a key rotation.
- How much theming and customization the generated artist/label storefront supports, and whether the hosted "build my site" convenience is an endonend platform responsibility or a separately run tool, given the platform must not become a required intermediary for artists who prefer to generate locally.
...
- The trigger point for moving the search index (and any other GCP-specific pieces) off a single persistent instance as the reference deployment grows, and at what scale endonend's own instance should move from Cloud Run to the Helm chart on GKE.
