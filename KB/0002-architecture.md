...
**This is the MVP architecture.** It defines the minimum system needed to validate the core mechanism: artist-owned static catalogs, a Union Platform that discovers, verifies, and indexes them, and basic playback. Playlists, fan sharing, radio/shuffle-style discovery, and broader cross-instance federation are explicitly out of scope here, per the non-goals and "What comes next" roadmap already in [0001-purpose.md](./0001-purpose.md), and will get their own specs once the MVP is validated.
...
- **Artist/label web frontend (optional, also generated static files).** An artist or label who wants their own branded web presence, not just a listing inside the Union Platform's site, can generate one from their manifest: a fully static HTML/CSS/JS storefront, produced once and hosted alongside their other static files, no server or ongoing build process required. See "Languages and stack" below for how the same frontend codebase produces both the Union Platform's site and this generated output.
- **Union Platform.** The central reference instance: a crawler/indexer that fetches and verifies artist manifests over plain HTTP, a catalog API, a search/discovery service, and the web and mobile clients. It never stores artist audio or image files itself, only metadata, pointers to artist-hosted files, and cache-only, TTL'd thumbnails.
- **Protocol.** The manifest file format and verification rules that let any artist's static files interoperate with any Union Platform instance, satisfying the cross-instance interoperability principle. Because there is no live service on the artist side to call, the protocol is fetch-and-verify: the platform pulls files and checks them, rather than a request/response API.
...
flowchart TB
    ...
    subgraph Union["Union Platform (central reference instance)"]
...
The Union Platform never touches the artist's audio, image, or storefront files directly; it only ever reads the manifest. Streams flow either straight from the artist's host or through the opt-in cache, which always relays plays back to the artist.

## Languages and stack

- **Union Platform backend (crawler, catalog API, search sync): Go.** Small container images, low resource footprint, and strong concurrency for fetching and verifying large numbers of artist manifests in parallel.
...
- **Web frontend: TypeScript and React** (e.g. Next.js), for server-side rendering and search-engine visibility on artist and discovery pages. A lower-stakes choice than the backend language, revisitable. The same frontend codebase runs in two modes: server-rendered against the Union Platform's catalog API for the central discovery site, and statically exported against a single artist's or label's manifest to produce a self-hostable storefront, plain HTML/CSS/JS with no server or Node.js runtime needed once generated. Generation can happen locally (a CLI step, matching the "no service" pattern used for the manifest itself) or through an optional hosted "build my site" convenience offered by the Union Platform, for artists who would rather not run anything locally at all.
...
Analytics stays simple by delegating collection to whatever already serves the bytes, rather than the Union Platform running a bespoke telemetry pipeline. There is one rule: raw, detailed listening data belongs to whoever served the play, by default the artist.
...
- **Cache-served plays: delegated back to the artist.** When a play is served from the optional media edge cache described above, the cache relays a play event back to the artist (a signed event or periodic aggregate report), so opting into caching never creates a gap in the artist's own numbers.
- **Optional play beacon, for artists without log access.** An artist can list a beacon URL of their choosing in their manifest. Web and mobile clients send a simple signed ping to it when a track starts playing. The artist picks where that ping goes, their own endpoint, a third-party analytics tool, or nowhere at all if they'd rather rely on host logs. The Union Platform never operates this endpoint on the artist's behalf; it only defines the ping format.
- **What the Union Platform aggregates centrally.** Only what discovery and ranking need: coarse, publicly visible listen counts per the "Verifiable, manipulation-resistant listening signals" principle. It does not centralize granular per-listener data, and whatever it does aggregate is published openly, not held as a private advantage, consistent with the radical transparency principle.
...
- **Union Platform:** a `docker-compose.yml` for a small, self-run union instance, and a Helm chart for larger-scale or cloud deployment of the reference instance. No hard dependency on any single cloud provider's proprietary services.
...
- **Media edge cache:** Cloud CDN in front of the artist-hosted origins is the concrete implementation of the optional media edge cache described earlier, for listeners hitting the union's reference instance.
...
- **Observability and cost transparency:** Cloud Logging and Cloud Monitoring for operational visibility, and GCP's billing export as a natural data source for the "compute/storage transparency reporting" the platform commits to publishing, per the open question already tracked in [0001-purpose.md](./0001-purpose.md).

## CI/CD

GitHub Actions, the default for an open-source, GitHub-hosted project: lint, test, and build on every pull request; build and push multi-arch container images on merge and on tag; sign container images (for example with cosign) so published images are themselves verifiable, matching the project's transparency principle. The two native mobile apps build on separate pipelines: the iOS app on macOS runners through Xcode, with its own signing and TestFlight/App Store release steps; the Android app on standard Linux runners through Gradle, with its own signing and Play Store release steps. Every pull request also runs the Go and Kotlin Multiplatform test suites against the shared conformance test vectors defined in 0003, so a canonicalization or signature bug in either implementation fails the build instead of surfacing later as a cross-platform verification mismatch. Deploying the union-operated reference instance to GCP is a separate pipeline step, authenticating via Workload Identity Federation rather than a long-lived service account key, and deploying the same signed images the public registry already published, not a GCP-specific build.

## Open questions and deferred decisions

These are intentionally left to future `Decision` or `Spec` docs rather than settled here:

- The exact manifest file schema, including the RFC 8785 canonicalization used for signing, is now defined in [0003-manifest.md](./0003-manifest.md); the remaining open item there is the literal machine-readable JSON Schema artifact.
- The specific minimum and maximum bounds the platform clamps a manifest's self-declared refresh TTL to, and what the platform default is when the field is omitted.
- The API key issuance and rate-limiting details for the submission endpoint (how a key is requested, its rate limit, whether it can be scoped to one manifest URL).
- How the submission endpoint validates the submitted URL itself, since a valid API key alone doesn't stop a key holder from pointing the crawler at an internal or otherwise unintended address (a server-side-request-forgery-style risk), separate from who is allowed to call the endpoint.
- How failure-notification email is handled reliably: extracting a contact email from a manifest whose signature itself may not have verified, avoiding repeated notification spam on a manifest that keeps failing the same way, and what happens on bounced or invalid addresses.
- The build and release tooling for the Kotlin Multiplatform module (packaging it as a native iOS framework via Kotlin/Native, versioning it alongside the two app releases). The conformance test vectors themselves are now defined in 0003; still open is the build-tooling mechanism for actually sharing one fixture set between a Go and a Kotlin test suite (a shared file format both languages parse, versus a small generator, versus something else).
- The manifest-generator/signer CLI tool's design is now defined in [0004-endtoend-artist-cli.md](./0004-endtoend-artist-cli.md); the remaining open item there is whether it should proactively notify previously-attested parties after a key rotation.
- How much theming and customization the generated artist/label storefront supports, and whether the hosted "build my site" convenience is a Union Platform responsibility or a separately run tool, given the platform must not become a required intermediary for artists who prefer to generate locally.
- The exact reporting mechanism for cache-served plays, so an artist who opts into the media edge cache still ends up with complete, trustworthy listening data rather than a gap between their own origin logs and the cache's.
- What the play beacon format includes and excludes, so a listener's play events can't be used to build a profile of that listener across every artist beacon they hit, even though each artist independently controls where their own beacon points.
- How payments actually work on a future roadmap: how money moves from listener to artist (subscriptions, pay-per-stream, tipping, or other models), which payment rails are used (traditional processors, crypto, or both), and who initiates settlement, given the split-transparency and central-entity-business-structure open questions already in [0001-purpose.md](./0001-purpose.md). Deferred to a dedicated future payments spec, not defined here.
- The trigger point for moving the search index (and any other GCP-specific pieces) off a single persistent instance as the reference deployment grows, and at what scale the union's own instance should move from Cloud Run to the Helm chart on GKE.
- How GCP's billing export is turned into the public, plain-language compute/storage transparency reporting the platform commits to, rather than raw billing data few listeners could interpret.
...
- [0004-endtoend-artist-cli.md](./0004-endtoend-artist-cli.md): the CLI tool that generates, signs, and validates that manifest.
