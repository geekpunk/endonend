...
| [0004](./0004-endonend-artist-cli.md) | endonend-artist-cli | Spec | Proposed | The Go CLI that generates, signs, and validates a manifest, interactive menu by default, flags for CI. |
| [0005](./0005-bandcamp-import.md) | Bandcamp import | Spec | Proposed | `import bandcamp` prefills `endonend.source.json` from an existing Bandcamp album page, downloading art/audio locally rather than linking to Bandcamp's hosting. |
| [0006](./0006-container-infrastructure.md) | Container infrastructure | Spec | Proposed | Generic container build, compose, Helm, and multi-arch signed-image publishing for the endonend platform backend; GCP-specific deployment deferred. |
...
| [0008](./0008-governance.md) | Governance | Decision | Accepted | Protocol and reference-instance governance is purely protocol-level, no foundation or central authority; conformance is mechanical, not a trust relationship. |
| [0009](./0009-federation.md) | Cross-instance federation | Spec | Draft | First-pass sketch: opt-in peer instance manifests and catalog-index feeds as leads only, every artist manifest still independently verified at the source. |
| [0010](./0010-mvp-scope.md) | MVP scope | Spec | Proposed | The concrete v1 cut: crawler + GraphQL catalog API, admin console for onboarding, a minimal web app reusing an existing album-player template, one docker-compose deployment, and the data/API shapes behind them. |
