...
Date: 2026-09-08
Type: Spec
References: [0001-purpose.md](./0001-purpose.md), [0003-manifest.md](./0003-manifest.md), [0004-endtoend-artist-cli.md](./0004-endtoend-artist-cli.md)
```

## Overview

An artist migrating off Bandcamp already has their catalog metadata (album title, tracklist, order, durations, release date, cover art) sitting on a page they control the content of, just not the hosting of. `endtoend-artist-cli import bandcamp <album-url>` reads that page and uses it to create or update `union.source.json`, the same source file [0004-endtoend-artist-cli.md](./0004-endtoend-artist-cli.md)'s `generate` command already reads, instead of making the artist retype every track title and duration by hand.
...
It deliberately does **not** write any `bandcamp.com`/`bcbits.com` URL into `union.source.json`. Two reasons, both tracing back to existing decisions rather than new ones:
...
`albumId` and `trackId` are derived by slugifying the album/track titles (e.g. `demo`, `demo-t1`); like every other source field, an artist can hand-edit these afterward, since [0004](./0004-endtoend-artist-cli.md) already treats `union.source.json` as human-editable.

## Interactive menu

The same behavior is also reachable as option 4, "Import an album from Bandcamp," on [0004](./0004-endtoend-artist-cli.md)'s top-level menu, consistent with that menu being the primary interface. It asks for the Bandcamp album URL, then, only if `union.source.json` doesn't exist yet, the same artist/label/URL/contact-email questions "Create or update your manifest" asks; an existing source file's identity is left untouched. It finishes by asking whether to download the cover art and audio locally now (default yes), then upserts the album exactly as the flag-driven command does.
...
- [0004-endtoend-artist-cli.md](./0004-endtoend-artist-cli.md): `union.source.json` and `generate`, which this command is a new way to bootstrap.
