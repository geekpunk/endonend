...
The core mechanism: **artists (or the label they choose to work with) host their own audio files, images, and other artifacts, and own their own serving and analytics.** The system does not sit in the artist's revenue or storage path. Instead, it functions like a an union, it represents and aggregates independent artists to give them collective reach, discoverability, and convenience for listeners, without ever owning or intermediating the artist's work or income the way streaming platform traditionally would.
...
The principles above commit the system to solving these, but the *how* is intentionally left to dedicated future specs rather than decided here:

- **Split transparency mechanism.** Exactly how and where a label/artist split is displayed to a listener, and what data backs that display.
- **Self-hosting accessibility.** What tooling, templates, or managed hosting options let a non-technical artist meet the artist-owned infrastructure principle without needing to run their own servers, given that principle and "artists do not need to be technologists" are in direct tension.
- **App store distribution constraints.** How native mobile clients published to Apple's App Store and Google Play stay consistent with the system's transparency and non-intermediary principles, given app store fees and review policies are a gatekeeper the system itself does not control.
- **Identity verification mechanism.** How the system verifies that an artist or label account is actually controlled by the artist or label it claims to represent, across decentralized, self-hosted instances, without creating a single central gatekeeper who can arbitrarily grant or revoke that verification.
- **History and audit trail integrity.** How an artist or label's public history (releases, affiliations, split changes) is recorded so it is tamper evident and cannot be quietly rewritten, given the system is deployed across independently run instances.
- **Listen verification / anti-manipulation mechanism.** How the system verifies or attests real listens and derives popularity/discovery signals from artist-hosted, self-reported serving infrastructure, given that the same decentralization that empowers artists also removes the platform's ability to unilaterally audit play counts the way a closed platform can.
- **Cross-instance interoperability / federation protocol.** How independently-run instances discover each other, share catalog/discovery data, and allow playback across instances.
- **Governance of the "union" aspect.** Whether there is a foundation, nonprofit, or elected body behind the reference instance and protocol standard, or whether governance is purely protocol-level with no central authority.
- **Central entity business structure.** What legal and business structure the organization behind the centralized reference/discovery instance takes (for example a nonprofit, a cooperative owned by member artists and labels, a foundation, or a transparently-margined for-profit), how it sustains itself financially, and where to start given the system's commitment to no legacy middleman economics.
- **MVP validation threshold.** How many artists and/or labels need to be onboarded to the reference instance to meaningfully validate the MVP, and what signals beyond raw count indicate the model is actually working for them.
- **Centralized union deployment architecture.** The deployment and scaling architecture for the centralized reference/discovery service itself, distinct from artist-hosted container deployments, including how it stays consistent with the compute/storage transparency principle.
- **Discovery/search centralization vs. decentralization.** How a convenient, centralized search and discovery experience coexists with an artifact/serving layer that is deliberately decentralized and artist-hosted.
- **Compute/storage transparency reporting.** Whether infrastructure usage is self-reported per instance, aggregated centrally, or both, and how it's verified.
